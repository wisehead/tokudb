...
在Fractal Tree删除一个对的方法是：调用函数 `toku_ft_root_put_msg` 给FT发送一个类型为 FT\_DELETE\_ANY 的message。其处理过程与insert类似，函数`msg_modify_ule`会判断message的类型，如果是FT\_DELETE\_ANY，就会给ULE的MVCC添加一个 XR\_DELETE 类型的provisional txn。
